...
void WTN6_PlayOneByte(uint8_t val)
{
	while (HAL_GPIO_ReadPin(WTN_GPIO_Port, WTN6040_BUSY_Pin) != 1);
	Line_2A_WTN6(val);
}
uint8_t WTN6_PlayArray(uint8_t arrlen, uint8_t *soundarr)
{
	uint8_t res = 0;
	uint8_t i = 0;
	for (i = 0; i < arrlen; i++)
	{
		WTN6_PlayOneByte(soundarr[i]);
		//osDelay(10);
...
uint8_t WTN6_PlayArray_1(uint8_t arrlen, uint8_t *soundarr)
{
	uint8_t res = 0;
	uint8_t i = 0;
	for (i = 0; i < arrlen; i++)
